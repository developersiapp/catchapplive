...
class StreamingNController extends Controller
{
    public function createUpdateStream(Request $request)
    {
        ...
        if ($club_stream)
        {
            ...
            /** UPDATE CLUB STREAM WITH UPDATE DETAILS ACCORDING TO MEDIUM */
            $url = "https://api.cloud.wowza.com/api/v1.3/live_streams/$stream_id";
            $method = "PATCH";
            $response = CurlHelper::CurlCommand($url, $method, $update_data);

            if ($encoder=='wowza_gocoder') {
    ...
    public function createStream(Request $request)
    {
        ...
        $broadcast_location = $request->input('broadcast_location');
        $encoder = $request->input('encoder');
//        $name = $request->input('stream_name');
        $transcoder_type = $request->input('transcoder_type');
